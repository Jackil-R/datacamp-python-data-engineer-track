...
import pandas as pd
csv = pd.read_csv('Datasets/world_ind_pop_data.csv')
feature_names = list(csv.columns)
row_vals = list(csv.values)[0]
# print(type(csv.values))
# print(list(csv.values[0,:]))
# print(list(csv.iloc[0,:]))
# print(len(list(csv[csv['Year'] == 1960].values)))
...
df_pop_ceb['Total Urban Population'] = [ int(total_pop * (urb_pop*0.01)) for total_pop, urb_pop in pops_list]
